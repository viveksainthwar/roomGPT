...
export async function POST(request: Request) {
  try {
    ...
    console.log("REPLICATE START RESPONSE:", jsonStartResponse); // debugging

    if (!jsonStartResponse?.urls?.get) {
      console.error("Missing urls.get in replicate response:", jsonStartResponse);
      return NextResponse.json(
    ...
    while (!restoredImage && attempts < maxAttempts) {
      console.log("Polling replicate for result...");
